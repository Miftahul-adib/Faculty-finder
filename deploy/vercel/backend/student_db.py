...
def _seed_phd_students():
    ...
    with get_conn() as con:
        for row in phd_data:
            cur = con.execute(
                "INSERT INTO phd_students (name,email,university,department,supervisor,research_area,bio,year_enrolled) VALUES (%s,%s,%s,%s,%s,%s,%s,%s) RETURNING id",
                row
            )
            phd_id = cur.fetchone()["id"]
