...
# ══════════════════════════════════════════════════════════════════════════
# No hf_login() here — Vercel's filesystem is read-only outside /tmp and
# login() tries to persist the token to ~/.cache/huggingface. The
# InferenceClient below takes the token directly per-request instead.
...
def setup_db():
    ...
    with get_conn() as con:
        if not col_exists(con, "faculty", "embedding"):
            con.execute("ALTER TABLE faculty ADD COLUMN embedding BYTEA")
            con.commit()
...
def setup_phd_db():
    ...
    with get_conn() as con:
        if not col_exists(con, "phd_students", "embedding"):
            con.execute("ALTER TABLE phd_students ADD COLUMN embedding BYTEA")
            con.commit()
